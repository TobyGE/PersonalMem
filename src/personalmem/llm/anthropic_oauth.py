...
Reads tokens from GuardClaw's storage at ``~/.guardclaw/oauth-tokens.json``
(same format used by ``server/cloud-judge.js``), auto-refreshes on 401, and
makes direct HTTP calls to ``https://api.anthropic.com/v1/messages`` with
the OAuth bearer header.

This bypasses litellm because litellm doesn't natively support Anthropic's
``Authorization: Bearer <oauth>`` flow — only ``x-api-key``.

The wrapper returns a litellm-shaped response object so the rest of the
pipeline (``extract_text``, ``extract_tool_calls``) keeps working unchanged.
"""
...
_TOKEN_FILE = Path.home() / ".guardclaw" / "oauth-tokens.json"
...
def _load_tokens() -> dict[str, Any]:
    if not _TOKEN_FILE.exists():
        raise RuntimeError(
            f"OAuth token file not found at {_TOKEN_FILE}. "
            "Run GuardClaw and authenticate with Claude first."
        )
